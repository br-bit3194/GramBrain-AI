...
class MandiDataLoader:
    ...
    async def get_commodity_prices(self, commodity: str, days_back: int = 7) -> Dict[str, Any]:
        ...
        try:
            ...
            # Filter for commodity (fuzzy match to support variations like "Onion (Desi)")
            search_key = commodity.strip().lower()
            commodity_data = [
                record for record in data["data"]
                if search_key and search_key in (record.get("commodity", "") or "").lower()
            ]

            # If no fuzzy matches found, fall back to exact match
            if not commodity_data:
                commodity_data = [
                    record for record in data["data"]
                    if record.get("commodity", "").lower() == search_key
                ]
